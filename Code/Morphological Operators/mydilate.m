function image = mydilate(I, SE)
% mydilate
    % Perform a dilation, defined as the maximum of the gray levels below the transposed strcturing element
	% Group 5.
    %
    ...
    %    'image'         	Output image
    %E.g.:
    %image = mydilate(rgb2gray(imread('00.001146.jpg')), strel('square', 5));
...
I = double(I);
SE = imrotate(SE,180);%Traspose structuring element
X = size(SE,1);
Y = size(SE,2);

%Compute the displacements based on the strc element
...
for i = 1:size(I,1)
    for j = 1:size(I,2)
        ...
        if( i-dii < 1 || i+did > size(I,1) || j-dji < 1 || j+djd > size(I,2))  %Estudi the boundaries 
            ...
            image(i,j) = max(max(I(i-dii:i+did, j-dji:j+djd).*SEn));%take max value of the pixels below the strc element
        else
        image(i,j) = max(max(I(i-dii:i+did, j-dji:j+djd).*SE)); %take max value of the pixels below the strc element
        end
    end
end
image = uint8(image);
